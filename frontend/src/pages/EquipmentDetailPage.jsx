...
export default function EquipmentDetailPage() {
  ...
  const [dealForm, setDealForm] = useState({
    buyerName: '', finalPrice: '', dueDate: '', terms: '', otp: '',
  });
  ...
  useEffect(() => {
    equipmentApi.getById(id).then((res) => setItem(res.data?.data)).catch(() => setItem(null)).finally(() => setLoading(false));
  }, [id]);
  ...
  if (loading || !item) return <Container className="py-4">{loading ? <Spinner /> : <p>{t('equipment.notFound')}</p>}</Container>;
    ...
    <Container className="py-4">
      <Card>
        ...
        <Card.Body>
          ...
          <p><strong>₹{item.pricePerDay}</strong> / day</p>
          <p className="text-muted">{item.location} {item.distanceKm != null && `• ${item.distanceKm.toFixed(1)} km away`}</p>
          <p className="small">Owner: {item.ownerName}</p>
          {user && !isOwner && (
            <>
              <Button className="me-2" onClick={() => setShowChat(true)}>{t('equipment.contact')}</Button>
              <Button variant="success" onClick={() => { setShowDeal(true); setOtpSent(false); setDealForm({ buyerName: user.name, finalPrice: item.pricePerDay, dueDate: '', terms: '', otp: '' }); }}>
        ...
        <Modal.Body>
          ...
          <Form onSubmit={handleConfirmDeal}>
            <Form.Group className="mb-2">
              <Form.Label>{t('agreement.buyerName')}</Form.Label>
              <Form.Control
                value={dealForm.buyerName}
                onChange={(e) => setDealForm((f) => ({ ...f, buyerName: e.target.value }))}
                required
              />
            </Form.Group>
            <Form.Group className="mb-2">
              <Form.Label>{t('agreement.finalPrice')}</Form.Label>
              <Form.Control
                type="number"
                step="0.01"
                value={dealForm.finalPrice}
                onChange={(e) => setDealForm((f) => ({ ...f, finalPrice: e.target.value }))}
                required
              />
            </Form.Group>
            <Form.Group className="mb-2">
              <Form.Label>{t('agreement.dueDate')}</Form.Label>
              <Form.Control
                type="date"
                value={dealForm.dueDate}
                onChange={(e) => setDealForm((f) => ({ ...f, dueDate: e.target.value }))}
              />
            </Form.Group>
            <Form.Group className="mb-2">
              <Form.Label>{t('agreement.terms')}</Form.Label>
              <Form.Control
                as="textarea"
            ...
            {!otpSent ? (
              <Button type="button" onClick={handleSendOtp}>{t('equipment.sendOtp')}</Button>
            ) : (
              <>
                <Form.Group className="mb-2">
                  <Form.Label>{t('auth.enterOtp')}</Form.Label>
                  <Form.Control
                    value={dealForm.otp}
                    onChange={(e) => setDealForm((f) => ({ ...f, otp: e.target.value }))}
                    maxLength={6}
                    required
                  />
                </Form.Group>
